...
public class CacheManager {
	...
	static {
		initCacheProvider("net.i77soft.ehcache.EhCacheProvider");
	}

	private static void initCacheProvider(String providerName)
	{
		try {
			CacheManager.provider = (CacheProvider)Class.forName(providerName).newInstance();
			CacheManager.provider.start();
			log.info("Using CacheProvider : " + provider.getClass().getName());
		}
		catch(Exception e) {
			log.fatal("Unabled to initialize cache provider:" + providerName + ", using ehcache default.", e);
			CacheManager.provider = (CacheProvider)new EhCacheProvider();
		}
	}

	private final static Cache _GetCache(String cacheName, boolean autoCreate)
	{
		if (provider == null) {
			provider = (CacheProvider)new EhCacheProvider();
		}
